...
int InitNet()
{
    ...
    add.sin_addr.s_addr = htonl("127.0.0.1");
    add.sin_port = htons("5060");
    if (-1 == bind(s_fd, (struct sockaddr*)(&add), sizeof(struct sockaddr))) {
        printf("bind failed\n");
        return -1;
    }
