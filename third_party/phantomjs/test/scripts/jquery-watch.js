...
(function($, undefined) {
        ...
        var rootWatchID = '__root_watch_element_id';
        var hasWatchedRoot = false;
        var cachedSelectorOptions = [];
        ...
        $.fn.watch = function(options) {
            ...
            var this$ = $(this);
            if (this$.length < 1) {
                waitUntilExists(this$.selector, opt);
                return this;
            }
            ...
            function waitUntilExists(selector, options) {
                if (!hasWatchedRoot) {
                    var root$ = $('html');
                    if (root$.length < 1) {
                        console.log('Error: cannot find dom element [html]');
                        return;
                    }
                    ...
                    root$.watch({
                        ...
                        callback : function(data, i, mutations) {
                            ...
                            if (nodeAdded) {
                                var newArray = [];
                                for (var i in cachedSelectorOptions) {
                                    var entry = cachedSelectorOptions[i];
                                    var e$ = $(entry.selector);
                                    if (e$.length > 0) {
                                        // this element shows up, apply the callbacks
                                        e$.watch(entry.options);
                                    } else {
                                        newArray.push(entry);
                                    }
                                }
                                cachedSelectorOptions = newArray;
                            }
                        } // callback
                    });
                }

                cachedSelectorOptions.push({
                    selector: selector,
                    options: options
                });
            }
